...
class DataMosher:
    ...
    def process_all_ranges(self):
        ...
        self.out_file = open(os.path.join(self.results_dir, f"{self.save_path}_moshed.avi"), 'wb')
        ...
        output_avi = os.path.join(self.results_dir, f"{self.save_path}_moshed.avi")
        final_output = os.path.join(self.results_dir, f"{self.save_path}_moshed.mp4")
        subprocess.call(
    ...
    def process_and_concat(self):
        ...
        for idx, (start, end) in enumerate(zip(self.start_frames, self.end_frames)):
            temp_output = os.path.join(self.results_dir, f"temp_mosh_{start}_{end}.mp4")
...
def main():
    ...
    parser.add_argument('--start_frames', nargs='+', type=int, required=True, help='List of start frames')
    parser.add_argument('--end_frames', nargs='+', type=int, required=True, help='List of end frames')
    parser.add_argument('--fps', '-f', default=30, type=int, help='fps to convert initial video to')
    parser.add_argument('--save_path', type=str, default='out', help="Base path to save processed video.")
    ...
    if len(args.start_frames) != len(args.end_frames):
        raise ValueError("start_frames and end_frames must have the same length.")
