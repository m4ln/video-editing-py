...
output_video = os.path.join(os.path.dirname(__file__), '..', 'results', args.save_path)
if not os.path.exists(video_path):
    raise FileNotFoundError(f"Video file {video_path} does not exist in the data directory.")
...
fps = args.fps
delta = args.delta
output_video = output_video if output_video.endswith('.mp4') else output_video + '.mp4'
